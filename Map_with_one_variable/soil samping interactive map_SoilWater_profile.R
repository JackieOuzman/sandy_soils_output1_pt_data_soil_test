...
soil_results_plus_location <- soil_results_plus_location %>%
  dplyr::select(all_of(id_col), "Soil_water_mm_profile", "SamplingDate", "ProfileDepth", "geometry") %>%
...
summary_tbl <- sampling_pts %>%
  ...
  summarise(
    n         = n(),
    mean_N    = round(mean(Soil_water_mm_profile, na.rm = TRUE), 2)  
  ) %>%
  arrange(zone)
#Format it as an HTML table for the map
summary_tbl_html <- kable(summary_tbl,
                  format   = "html",
                  col.names = c("Zone", "Count", "Mean Soil water mm"),
                  align    = "c") %>%
